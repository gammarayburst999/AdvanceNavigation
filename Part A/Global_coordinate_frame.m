...
%Obtain an estimate of the position (Using Kalman Filter) and orientation of the platform at that time
%Perform feature extraction, for detecting the OOIs
%Express the currently detected OOIs’ positions in a global coordinate frame
...
figure(2);
clf();
MyGUIHandles3.handle1 = plot(0,0,'+r');      % to be used for showing the laser points
hold on
axis([-4,6,0,7]);                          % focuses the plot on this region (of interest, close to the robot)
xlabel('X-axis');
ylabel('Y-axis');
MyGUIHandles3.handle2 = title('');
MyGUIHandles3.handle3 = plot(0,0,'.b','MarkerSize',12);
legend('Landmarks(map)','Robot')
zoom on ;  grid on;
hold off;
%------------------ GUI ---------------
...
while 1
    ...
    MyProcessingOfScan(scan_i,MyGUIHandles2,MyGUIHandles3,i,final_pose);   % some function to use the data...
...
%-----------------------------------------
function MyProcessingOfScan(scan,mh2,mh3,i,position_data)
global ABCD;
